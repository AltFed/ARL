...
#include <wait.h>
#define MAXLINE 4096
// implementare un mutex per prendere il controllo e comunicare con il proprio
// client dato che i thread modificano le variabili globali
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// variabili globali
int TOs = 0;
...
void sig_time(int signo) {
  ...
  while (k < seqnum) {
    ...
    if (!strcmp(pkt.pl, "slow")) {
      ...
      fflush(stdout);
      bytes_psecond = 100;
      if (setsockopt(fd, SOL_SOCKET, SO_MAX_PACING_RATE, &bytes_psecond,
                     sizeof(bytes_psecond)) < 0) {
        perror("Error setsockopt");
        exit(1);
      }
