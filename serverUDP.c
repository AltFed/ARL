    ...
    if (n == 0) {
      ...
      while (k < seqnum && stay == true ) {
        ...
        while (swnd < CongWin && swnd < lt_rwnd && stay == true) {
          printf("congwin %d, swnd: %d k: %d retr ack :%d lt_rwnd: %d\n", CongWin, swnd, k,retr[k].ack,lt_rwnd);
          fflush(stdout);
...
void rcv_put(int sockfd, char *file) {
  ...
  while (stay) {
    ...
    if ((k = recvfrom(sockfd, rcv_buff, 4098, 0, (struct sockaddr *)&addr,
                      &addrlen)) < 0) {
      perror("errore in recvfrom");
      exit(1);
    }
...
void send_list(int sockfd) {
  ...
  char fin_buff[MAXLINE + 10]; // poi modificare il 10 deve essere comunque
                               // maggiore di MAXLINE di almeno 3/4
  ...
  while (stay) {
    ...
    // qui vedo se il buff che invio al client è pieno o meno se si vado avanti
    // altrimenti inserisco il terminatore
    if (strlen(snd_buff) < MAXLINE) {
      ...
      // -1: perchè copio i bit ma devo togliere il numero  che vengono contati
      // dallo strlen
      if ((sendto(sockfd, fin_buff, strlen(fin_buff), 0,
                  (struct sockaddr *)&addr, addrlen)) < 0) {
        perror("errore in sendto");
        exit(1);
      }
      ...
      // controllo se è arrivato un ack dal receiver in caso gestisco gli errori
      // non bloccante
      if ((recvfrom(sockfd, rcv_buff, sizeof(rcv_buff), MSG_DONTWAIT,
                    (struct sockaddr *)&addr, &addrlen)) < 0) {
        perror("errore in recvfrom");
        exit(-1);
      }
...
int main(int argc, char **argv) {
  if (argc != 6) {
    fprintf(stderr, " utilizzo:<pkt loss probab P , TO[s] , TO[ms] , "
                    "SERV_Port, Inserire numero dei figli> \n");
    exit(1);
  }
  ...
  while (nchildren < 1) {
    u++;
    char temp[100];
    printf("Numero figli deve essere >= 1");
    fscanf(stdin, "%s", temp);
    nchildren = atoi(temp);
    if (u > 5) {
      exit(1);
