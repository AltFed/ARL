...
void *rcv_list(void *sd){
  ...
  while (stay) {
    ...
    if (maxackrcv < lt_ack_rcvd) {
      CongWin++;
      swnd=seqnum-lt_ack_rcvd;
      printf("RCV_CONG :: new swnd value =%d  lt_ack=%d  seqnum = %d maxack =%d\n",swnd,lt_ack_rcvd,seqnum,maxackrcv);
      fflush(stdout);
      maxackrcv = lt_ack_rcvd;
    }
    if (pkt.finbit == 1 && pkt.ack == seqnum) {
  ...
  while (stay) {
    ...
    if (n == 0){
      ...
      while (k < seqnum && stay == true) {
        ...
        lt_rwnd = pkt.rwnd;
        lt_ack_rcvd = pkt.ack;
        printf("lt_ack_rcvd %d",lt_ack_rcvd);
        fflush(stdout);
        if (maxackrcv < lt_ack_rcvd){
          CongWin++;
          swnd=seqnum-lt_ack_rcvd;
...
void send_get(char *str,int sockfd) {
  ...
  while (stay) {
    ...
    // dell'ultimo pkt inviato poi continuo
          ...
          pkt.pl[dimpl] = '\0';
          retr[i] = pkt;
          printf("\n\n\nSERVER send last pkt %d %s  ", i,pkt.pl);
          prob = (double)rand() / RAND_MAX;
  ...
  if (pthread_create(&thread_id, NULL, rcv_list, sockfd) != 0) {
    ...
    while ((file = readdir(directory)) != NULL && swnd < CongWin && stay == true && swnd < lt_rwnd){
      ...
      if (strcmp(file->d_name, ".") != 0 && strcmp(file->d_name, "..") != 0) {
        ...
        if(strlen(pkt.pl) == MAXLINE){// se ho il pl pieno invio il pkt 
          printf("%s\n\n",pkt.pl);
          fflush(stdout);
          swnd++;
          seqnum++;
...
void *child_main(void *k) {
  int i=k;
  int listenfd;
  int n_port=SERV_PORT+i;
  ...
  addr.sin_port = htons(n_port); /* numero di porta del server la uso per
  ...
  for (;;) {
    //forse invece di un booleano posso usare i mutex conviene? 
    if(stop[i-1]){
    send_control(listenfd,i); /* processa la richiesta */
    }
  }
}
...
int main(int argc, char **argv) {
  if (argc != 6) {
    fprintf(stderr, " utilizzo:<pkt loss probab P , TO[s] , TO[ms] , "
                    "SERV_Port, Inserire numero dei thread> \n");
    exit(1);
  }
  ...
  //fare nel padre un controllo su quale elemento è libero ovvero false e assegno il client e quest ultimo
  int u = 0,n=0;
  ...
  // qui alloco un area di memoria per mantenere i pid dei thread
  pids = (pthread_t*)calloc(nchildren, sizeof(pthread_t));
  if (pids == NULL) {
  ...
  for (int i = 0; i < nchildren; i++) {
    tep=i+1;
    stop[i]=false;
    // inserisco nell array pids i pid dei thread
    if(pthread_create(&pids[i],NULL,child_main,tep) != 0){
      perror("pthread main error");
      exit(1);
    }
  }
