...
// ovvero R06
 ...
 //RE SENZA PI/2 - BETA 
  ...
  //Re[0][1]=(-cos(alfa)*cos(beta)*sin(theta)-sin(alfa)*cos(theta));
  //Re[0][2]=(cos(alfa)*sin(beta));
  
  //Re[1][0]=(sin(alfa)*cos(beta)*cos(theta)+cos(alfa)*sin(theta));
  //Re[1][1]=(-sin(alfa)*cos(beta)*sin(theta)+cos(alfa)*cos(theta));
  //Re[1][2]=(sin(alfa)*sin(beta));
  
  //Re[2][0]=(-cos(theta)*sin(beta));
  //Re[2][1]=(sin(beta)*sin(theta));
  //Re[2][2]=(cos(beta));
